...
export class ValidationMessageDirective implements OnInit, OnDestroy {
  ...
  constructor(private ngControl: NgControl, private renderer: Renderer2, private elemRef: ElementRef) {
  ...
  private showHideErrors() {
    ...
    if (controlErrors) {
      ...
      this.renderer.insertBefore(this.elemRef.nativeElement.parentNode, this.errorLabel, this.elemRef.nativeElement.nextSibling);
      return;
    }
    if (this.errorLabel.parentNode === this.elemRef.nativeElement.parentNode) {
      this.renderer.removeChild(this.elemRef.nativeElement.parentNode, this.errorLabel);
    }
  }
}
